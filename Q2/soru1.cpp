...
{   
    ...
    for (int i = m; i < buffer_size-1; i++) { 
        ...
        //cout <<"i :"<< i <<" product :" << maxProd <<" sum :" <<sum<<endl; 

        
        // Multiply with the current digit and divide by 
        // the first digit of previous window 
        int s1 = int(buffer[i]);
        int s2 = int(buffer[i-m]);

        if (s2 == 0){
            product = 1;
            for (int j = 1; j<(m+1); j++ ){
                product *= int(buffer[i-m+j]);
            }
        }
        else{
            product = product * s1 / s2;     
        }
    ...
    /*for (int a = startPoint-3; a<startPoint+15; a++)
    cout << a <<": "<< int(buffer[a])<<endl; */
