...
void maxProductSum(uint8_t *buffer, int buffer_size, int m) 
{ 
    int product = 1;
    int maxProd = product; 
    int startPoint = 0;
    ...
    for (int i = 0; i < m; i++) {
        product *= int(buffer[i]); 
    } 
  

    // Start traversing the next element 
    for (int i = m; i < buffer_size-1; i++) { 
        
        int last_number = int(buffer[i]);
        int first_number = int(buffer[i-m]);

        // Check zero division
        if (first_number == 0){
            product = 1;
            for (int j = 1; j<(m+1); j++ ) product *= int(buffer[i-m+j]);            
        }
        else{
            product = product * last_number / first_number;     
        }

  
        // Update max product 
        if (product > maxProd){
            cout << "product: " << product << " maxProd: " << maxProd << endl;
            maxProd = product;
            startPoint = i-m+2;
            endPoint = i+1;
        }
...
{ 
    ...
    maxProductSum(buffer, buffer_size, m); 
